...
[3.2. FN_CALL_WRITE: Call a FujiNet function sending data to the device](#32-fn_call_write-call-a-fujinet-function-sending-data-to-the-device)

[3.3. FN_CALL_READ: Call a FujiNet function reading data from the device](#33-fn_call_read-call-a-fujinet-function-reading-data-from-the-device)
...
### 3.2. FN_CALL_WRITE: Call a FujiNet function sending data to the device
...
This routine calls a FujiNet firmware function that reads data from the FujiNet device into MSX memory. The received data will be written to memory starting at the address specified in buffer, and length specifies the maximum number of bytes to receive.  If there is no data to be received then it is prefered to use FN_CALL_WRITE.
